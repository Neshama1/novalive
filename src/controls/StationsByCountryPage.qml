...
Maui.Page {
    ...
    Component.onCompleted: {
        getStations(CountryBackend.code[0])
    }
            ...
            MouseArea {
                ...
                Menu {
                    ...
                    MenuItem {
                        ...
                        onTriggered: {
                            if (!savedStation()) {
                                favoritesModel.append(stationsByCountryModel.get(rightStationIndex))
                            }
                            sortModel()
                        }
                    }
                    MenuItem {
                        text: "Visit website"
                        onTriggered: {
                            Qt.openUrlExternally(stationsByCountryModel.get(rightStationIndex).homepage)
                        }
                    }
                }
            }
    ...
    function savedStation() {
        ...
        for (i = 0; i < favoritesModel.count ; i++) {
            if (stationsByCountryModel.get(rightStationIndex).stationuuid == favoritesModel.get(i).stationuuid) {
                return true
            }
        }
        return false
