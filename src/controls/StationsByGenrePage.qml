...
Maui.Page {
    ...
    Component.onCompleted: {
        getStations(GenresBackend.genres[0])
    }
    ...
    function getStations(genre) {
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState === XMLHttpRequest.HEADERS_RECEIVED) {
                ...
                stationsByGenreCount = stationsByGenreModel.count
            }
        }
        xhr.open("GET", "https://de1.api.radio-browser.info/json/stations/search?limit=999&tagList=" + genre + "&hidebroken=true&order=clickcount&reverse=true");
        xhr.send();
    }
    ...
    GridView {
        ...
        delegate: Rectangle {
            ...
            color: stationMouse.hovered ? (Maui.ColorUtils.brightnessForColor(Maui.Theme.backgroundColor) == Maui.ColorUtils.Light ? Qt.darker(Maui.Theme.alternateBackgroundColor,1.04) : Qt.lighter(Maui.Theme.backgroundColor,1.2)) : Qt.lighter(Maui.Theme.alternateBackgroundColor,1.02) /*Qt.darker(Maui.Theme.alternateBackgroundColor,1.01)*/
            radius: 4
            Rectangle {
                width: parent.width
            ...
            MouseArea {
                ...
                Menu {
                    ...
                    MenuItem {
                        ...
                        onTriggered: {
                            if (!savedStation()) {
                                favoritesModel.append(stationsByGenreModel.get(rightStationIndex))
                            }
                            sortModel()
                        }
                    }
                    MenuItem {
                        text: "Visit website"
                        onTriggered: {
                            Qt.openUrlExternally(stationsByGenreModel.get(rightStationIndex).homepage)
                        }
                    }
                }
            }
    ...
    function savedStation() {
        ...
        for (i = 0; i < favoritesModel.count ; i++) {
            if (stationsByGenreModel.get(rightStationIndex).stationuuid == favoritesModel.get(i).stationuuid) {
                return true
            }
        }
        return false
