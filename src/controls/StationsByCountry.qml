...
Maui.Page {
    ...
    Component.onCompleted: {
        ...
        getStations(CountryBackend.code[countriesCurrentIndex])
    }

    PropertyAnimation {
        id: opacityAnimation
        target: stationsByCountryPage
        properties: "opacity"
        from: 0
        to: 1.0
        duration: 250
            ...
            Maui.SwipeBrowserDelegate
            {
                ...
                quickActions: [
                    ...
                    Action
                    {
                        icon.name: "go-home"
                        onTriggered: {
                            Qt.openUrlExternally(stationsByCountryModel.get(index).homepage)
                        }
                    }
                ]
            }
    ...
    function savedStation() {
        ...
        for (i = 0; i < favoritesModel.count ; i++) {
            if (stationsByCountryModel.get(list.currentIndex).stationuuid == favoritesModel.get(i).stationuuid) {
                return true
            }
        }
        return false
