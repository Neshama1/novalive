...
Maui.Page {
    ...
    Component.onCompleted: {
        getStations(LanguageBackend.language[0])
    }
            ...
            MouseArea {
                ...
                Menu {
                    ...
                    MenuItem {
                        ...
                        onTriggered: {
                            if (!savedStation()) {
                                favoritesModel.append(stationsByLanguageModel.get(rightStationIndex))
                            }
                            sortModel()
                        }
                    }
                    MenuItem {
                        text: "Visit website"
                        onTriggered: {
                            Qt.openUrlExternally(stationsByLanguageModel.get(rightStationIndex).homepage)
                        }
                    }
                }
            }
    ...
    function savedStation() {
        ...
        for (i = 0; i < favoritesModel.count ; i++) {
            if (stationsByLanguageModel.get(rightStationIndex).stationuuid == favoritesModel.get(i).stationuuid) {
                return true
            }
        }
        return false
